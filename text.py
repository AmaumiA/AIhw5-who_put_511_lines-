...
class MultimodalModel(nn.Module):
    ...
    def forward(self, text, image):
        ...
        # image_output = image_output.view(32, -1)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        black_image = np.zeros_like(img, dtype=np.uint8)
        black_image_pil = Image.fromarray(black_image)
        image_data = np.asarray(black_image_pil, dtype='float32').transpose(2, 0, 1)
        image = torch.Tensor(image_data)
...
with open('./实验五数据/test_text.txt', 'w') as f:
    f.writelines(output_lines)
with open('./实验五数据/test_text.txt', 'r', encoding='utf-8') as file:
    content = file.read()
...
with open('./实验五数据/test_text.txt', 'w', encoding='utf-8') as file:
